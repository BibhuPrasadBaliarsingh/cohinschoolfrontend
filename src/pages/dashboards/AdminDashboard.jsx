    ...
    <DashboardLayout>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-purple-900/40 via-navy-900 to-navy-950 border border-purple-500/30 backdrop-blur-xl mb-8 relative overflow-hidden">
        ...
        <h1 className="font-display text-3xl font-extrabold text-white mb-2">
          System Governance & Security Desk
        </h1>
        <p className="text-white/70 text-sm max-w-2xl">
          Logged in as <strong className="text-purple-300">{user?.name}</strong> ({user?.email}). MongoDB Cloud Database sync is active with encrypted JWT RBAC session tokens.
        </p>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <option value="all" className="bg-navy-900">All Roles</option>
              <option value="admin" className="bg-navy-900">Admin</option>
              <option value="principal" className="bg-navy-900">Principal</option>
              <option value="teacher" className="bg-navy-900">Teacher</option>
              <option value="student" className="bg-navy-900">Student</option>
                ...
                <tr key={i} className="hover:bg-white/5 transition">
                  ...
                  <td className="p-3.5">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase border ${
                      ...
                      u.role === 'principal' ? 'bg-amber-500/20 text-amber-300 border-amber-500/40' :
                      u.role === 'teacher' ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' :
                      u.role === 'student' ? 'bg-blue-500/20 text-blue-300 border-blue-500/40' :
